...
function linkageform($linkageid = 0, $defaultvalue = 0, $id = 'linkage', $level = 3, $required) {
	...
	for ($i = 1; $i <= $level; $i++) {
		...
		$string .='<select class="rcms-select-' . $id . '" name="' . $id . '-' . $i . '" id="' . $id . '-' . $i .'" width="100" ' . $style . $required . '><option value=""> -- </option></select>&nbsp;&nbsp;';
	}
	//$default_txt="['福建省','厦门市']";
    //if(!defined('RCMS_LINKAGE_INIT_LD')) {
	//	define('RCMS_LINKAGE_INIT_LD', 1);
		$string .= '<script type="text/javascript" src="' . SITE_THEME . 'js/jquery.ld.js"></script>';
	//}
	$string .= '<script type="text/javascript">
				$(function(){
					var $ld5_' . $id . ' = $(".rcms-select-' . $id . '");					  
					$ld5_' . $id . '.ld({ajaxOptions:{"url":"' . SITE_URL . 'index.php?c=api&a=linkage&id=' . $linkageid . '"},defaultParentId:0,texts:' . $default_txt . '});	 
...
function linkageform_area($linkageid = 0, $defaultvalue = 0, $id = 'linkage', $level = 3, $required) {
	...
	$string .= $defaultvalue ? '<input type="hidden" name="' . $id . '"  id="fc_' . $id . '" value="' . $defaultvalue . '">' : '<input type="hidden" name="' . $id . '"  id="fc_' . $id . '" value="">';
	for ($i = 1; $i <= $level; $i++) {
		$style   = $i > $default_lev ? 'style="display:none"' : '';
		$required = $i == 1 && $required ? ' required' : '';
		$string .='<select class="rcms-select-' . $id . '" name="' . $id . '-' . $i . '" id="' . $id . '-' . $i .'" width="100" ' . $style . $required . '><option value=""> -- </option></select>&nbsp;&nbsp;';
