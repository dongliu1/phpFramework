...
function html_to_data($data, $file) {
	$data = is_array($data) ? $data : array('rcms_html_to_data' => $data);
...
function downfile($url) {
	return url('api/down', array('file' => str_replace('=', '', base64_encode(fn_authcode(array('rcms' => $url), 'ENCODE')))));
}
...
function _thumb($img, $width = null, $height = null) {
	return url('api/thumb', array('img' => str_replace('=', '', base64_encode(fn_authcode(array('rcms' => $img), 'ENCODE'))), 'width' => $width, 'height' => $height));
}
 ...
 * @return mixed
 */
function new_html_special_chars($arr) {
	if(!is_array($arr)) return htmlspecialchars($arr);
	foreach($arr as $key => $val) $arr[$key] = new_html_special_chars($val);
	return $arr;
}
...
function textarea2json($str){
	$str	= str_replace(chr(13), "", $str);
	$rows	= explode(chr(10), $str);
	$json	=array();
	foreach ($rows as $row) {
		if(strpos($row,"|")==false){
			array_push($json,$row);
...
function array2string($data, $isformdata = 1) {
	return json_encode($data, JSON_UNESCAPED_UNICODE);////php 5.4 以上版本
	////rox php5.3 及以前版本 return preg_replace("/\\\u([0-9a-f]{4})/ie", "iconv('UCS-2BE', 'UTF-8', pack('H4', '$1'))", json_encode($data));
	/*if($data == '') return '';
 ...
 * 获取当前站点的联动菜单缓存数据
 */
function get_linkage_data($isAssets=false) {
	$cache = new cache_file();
    $linkage=$isAssets?'assets':'linkage';
	$data  = $cache->get($linkage);
	$site  = $cache->get($linkage.'_' . App::get_site_id());
 ...
 * 调用联动菜单数据
 */
function linkagedata($keyid, $id = 0,$isAssets=false) {
	if (empty($id)) return false;
    $datas = get_linkage_data($isAssets);
	$data  = $datas[$keyid];
	if (empty($data)) return false;
	if ($id == $keyid) {
	    unset($data['data']);
...
function assetsdata($keyid) {
	if (empty($keyid)) return false;
    $datas = get_linkage_data(true);
	$data  = $datas[$keyid];
	if (empty($data)) return false;
	return $data;
}
function assetslist($keyid, $id = 0,$isAssets=false) {
	return linkagelist($keyid, $id,true);
}
function assetsform($linkageid = 0, $defaultvalue = 0, $id = 'linkage', $level = 3, $required){
	$isAssets=true;
    $data  = get_linkage_data($isAssets);
	$datas = $data[$linkageid];
	$infos = $datas['data'];
	$default_txt = '1';
	$default_lev = 1;
	if($defaultvalue!=0) {
		$default_txt = menu_linkage_level($defaultvalue, $linkageid, $infos,$result,$isAssets);
		$default_lev = substr_count($default_txt, ' > ');
		$default_txt = '["' . str_replace(' > ', '","', $default_txt) . '"]';
	}
	$string .='<input type="text" class="button easyui-combotree" name="data[' . $id . ']"  id="fc_' . $id . '" value="' . $defaultvalue . '" data-options="url:\' '.url("api/linkagedata",array("id"=>1,"isAssets"=>1,"parent_id"=>0)).'\',method:\'get\',required:true,loadFilter:function(rows){return treedata_convert(rows);}" />'; 

	return $string;
	//return linkageform($linkageid, $defaultvalue, $id, $level, $required,true);
}

/**
 * 调用联动菜单数据
 */
function linkagelist($keyid, $id = 0,$isAssets=false) {
    $datas = get_linkage_data($isAssets);
	$list  = array();
	if($keyid==0){
		foreach ($datas as $k => $t) {
			if (0 == $t['keyid']) {
			    if ($t['arrchilds']) $t['arrchilds'] = explode(',', $t['arrchilds']);
				$t['data']=[];
				//list($t['name'],$t['ename'])=explode("|",$t['name']);
			    $list[$k] = $t;
			}
		}
		return $list;
 ...
 * 联动菜单面包屑导航
 */
function linkagepos($keyid, $id, $urlrule, $s = ' > ',$isAssets=false) {
	$ids  = linkageids($keyid, $id,$isAssets);
	foreach ($ids as $_id) {
	    $data = linkagedata($keyid, $_id,$isAssets);
		$name = explode("|",$data['name'])[0];
		$str .= $urlrule ? "<a href=\"" . str_replace('{linkageid}', $data['id'], $urlrule) . "\">" . $name . "</a>" : $name;
        if ($id != $_id) $str .= $s;
    }
	return $str;
}
 ...
 * 调用该菜单的上级菜单id集合
 */
function linkage_ids($keyid, $id, $ids = '',$isAssets=false) {
    $datas = get_linkage_data($isAssets);
    $data  = $datas[$keyid]['data'][$id];
    $ids   = $id . ','; 
    if ($data['parentid']) $ids .= linkage_ids($keyid, $data['parentid'], $ids,$isAssets);
    return $ids;
}
 ...
 * 调用该菜单的上级菜单id集合(数组返回)
 */
function linkageids($keyid, $id,$isAssets=false) {
    $ids = linkage_ids($keyid, $id,$isAssets);
 ...
 * required 必填
 */
function linkageform($linkageid = 0, $defaultvalue = 0, $id = 'linkage', $level = 3, $required,$isAssets=false) {
	$linkage=$isAssets?'assets':'linkage';
    $data  = get_linkage_data($isAssets);
	$datas = $data[$linkageid];
	$infos = $datas['data'];
	...
	//return json_encode($string,256);
	for ($i = 1; $i <= $level; $i++) {
		$style   = $i > $default_lev ? 'style="display:none"' : '';
		$required = $i == 1 && $required ? ' required' : '';
		$string .='<select class="rcms-select-' . $id . '" name="' . $id . '-' . $i . '" id="' . $id . '-' . $i .'" width="100" ' . $style . $required . '><option value="-'.$i.'"> -- </option></select>&nbsp;&nbsp;';
	}
	//$default_txt="['福建省','厦门市']";
    //if(!defined('RCMS_LINKAGE_INIT_LD')) {
	//	define('RCMS_LINKAGE_INIT_LD', 1);
	$string .= '<script type="text/javascript" src="' . JLIB_PATH . 'cascadelink/jquery.ld.js"></script>';
	//}
	$string .= '<script type="text/javascript">
				$(function(){
					var $ld5_' . $id . ' = $(".rcms-select-' . $id . '");					  
					$ld5_' . $id . '.ld({ajaxOptions:{"url":"' . SITE_URL . 'index.php?c=api&a=linkage&isAssets='.$isAssets.'&id=' . $linkageid . '"},defaultParentId:0})	 
					var ld5_' . $id . '_api = $ld5_' . $id . '.ld("api");
					ld5_' . $id . '_api.selected(' . $default_txt . ');
					$ld5_' . $id . '.bind("change",onchange);
					function onchange(e){
						var $target = $(e.target);
						var index = $ld5_' . $id . '.index($target);
						$("#' . $id . '-' . $i . '").remove();
						$("#fc_' . $id . '").val($ld5_' . $id . '.eq(index).show().val());
						index ++;
						$ld5_' . $id . '.eq(index).show();
					}
				})
	</script>';
	return $string;
...
//省市县数据 
function linkageform_area($linkageid = 0, $defaultvalue = 0, $id = 'linkage', $level = 3, $required) {
	...
	//$string .= $defaultvalue ? '<input type="text" name="' . $id . '"  id="fc_' . $id . '" value="' . $defaultvalue . '">' : '<input type="text" name="' . $id . '"  id="fc_' . $id . '" value="">';
	
	//后期处理defaultvalue
	$string .= $defaultvalue ? '<input name="' . $id . '"  id="fc_' . $id . '" autocomplete="off" type="text" value="请选择/输入城市名称" class="city_input inputFocus proCityQueryAll proCitySelAll" ov="请选择/输入城市名称">':'<input name="' . $id . '"  id="fc_' . $id . '" autocomplete="off" type="text" value="请选择/输入城市名称" class="cascadeSelect inputFocus proCityQueryAll proCitySelAll" ov="请选择/输入城市名称">';
	
	$string .= '<link href="' . EXT_PATH . 'cascadeselect/css/cityLayout.css" type="text/css" rel="stylesheet">';

	$string .= '<script type="text/javascript" src="' . EXT_PATH . 'cascadeselect/js/cascade.js"></script>';
	return $string;
}
 ...
 * 联动菜单层级
 */
function menu_linkage_level($linkageid, $keyid, $infos, $result = array(),$isAssets=false) {
	if($infos){
	if (array_key_exists($linkageid, $infos)) {
		$result[] = $infos[$linkageid]['id'];
		return menu_linkage_level($infos[$linkageid]['parentid'], $keyid , $infos, $result,$isAssets);
	}}
	if($result==null){
		return "";
	}else{
