...
function content_radio($name, $content = '', $field = '') {
    ...
    $select	= explode(chr(10), $setting['content']);
    $str	= '';
	$str.= '<script type="text/javascript" src="' . SITE_THEME . 'js/jquery-ui.min.js"></script>';
...
function content_date($name, $content = '', $field = '') {
	...
	if (!defined('RCMS_DATE_LD')) {
		...
		<script type="text/javascript" src="' . SITE_THEME . 'js/jquery-ui.min.js"></script>';
...
//返回联动菜单值
function text_linkage($name, $content = '', $field = '') {	
	$setting	= isset($field['setting']) ? string2array($field['setting']) : $field;	//配置
    $content	= is_null($content) ? get_content_value($setting['default']) : $content;
	return linkagepos($setting['id'], $content, null, '') ;
}
