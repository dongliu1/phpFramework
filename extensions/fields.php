...
function form_input($setting = '') {
	...
	<tr> 
      <td>' . lang('a-mod-95') . ' ：</td>
      <td><input type="text" class="input-text" size="30" value="' . (isset($setting['default']) ? $setting['default'] : '') . '" name="setting[default]" id="setting_default">
	  <select onchange="$(\'#setting_default\').val(\'{M:\'+this.value+\'}\')" name="_member_field"><option value=""> -- </option><option value="username"> 会员登录名 </option><option value="email"> 会员邮箱 </option><option value="nickname"> 昵称 </option><option value="lianxidianhua"> 电话 </option></select>
	  </td>
    </tr>
    </tbody>
	</table>';
...
function form_select($setting = '') {
	...
	<tr> 
      ...
      <td><textarea name="setting[content]" style="width:195px;height:100px;" class="text">' . (isset($setting['content']) ? stripslashes($setting['content']) : '') . '</textarea>
...
function form_assets($setting = '') {
	$link = ($isAssets)?Controller::model('assets'):Controller::model('linkage');
	...
	//return form_linkage($setting,true);
}
function form_linkage($setting = '',$isAssets=false) {
    $link = ($isAssets)?Controller::model('assets'):Controller::model('linkage');
	$data = $link->where('keyid=0')->select();
	$str  = '
    <table width="98%" cellspacing="1" cellpadding="2">
...
function content_input($name, $content = '', $field = '') {
    ...
    return '<input type="text" value="' . $content . '" class="input-text" id="fc_'.$name .'" name="data[' . $name . ']" ' . $style . ' ' . $required . $pattern . ' />';
}
...
function content_password($name, $content = '', $field = '') {
	...
	$required	= isset($field['not_null']) && $field['not_null'] ? ' required' : '';	//必填字段
    $style		= isset($setting['size']) ? " style='width:" . ($setting['size'] ? $setting['size'] : 150) . "px;'": ''; 
    return '<input type="password" value="' . $content . '" class="input-text" id="fc_'.$name .'" name="data[' . $name . ']" ' . $style . ' ' . $required . ' />';
}
...
function content_textarea($name, $content = '', $field = '') {
    ...
    return '<textarea style="' . $style . '" id="fc_'.$name .'" name="data[' . $name . ']" ' . $required . $pattern . '>' . $content . '</textarea>';
}
...
function kindeditor_editor($name, $content = '', $field = '') {
	...
	//if (!defined('RCMS_EDITOR_LD')) {
		...
		';
	//	define('RCMS_EDITOR_LD', 1);//防止重复加载JS
	//}
		...
		$(function() {
			KindEditor.create('#fc_" . $id . "', { 
				...
				items : [
				    ...
				    'removeformat', '|', 'justifyleft', 'justifycenter', 'justifyright', '|', 'image','media', 'link' " . $page . "
				]
			});
		});
		</script>";
...
function ueditor_editor($name, $content = '', $field = '') {
	...
	//if (!defined('RCMS_EDITOR_LD')) { //加载相关js
	    $str.= '
	...
	if ($type) { //完整模式
		...
		<script type=\"text/javascript\">
			var editorOption = {
				...
				imageUrl:\"" . url('attachment/ueditor_upload') . "&dir=image&document=\"+rcms_admin_document,
                imagePath:\"" . SITE_PATH . "\",
				imageManagerUrl:\"" . url('attachment/ueditor_manager', array('dir' => 'image')) . "\",
                imageManagerPath:\"" . SITE_PATH . "\",
				fileUrl:\"" . url('attachment/ueditor_upload') . "&dir=file&document=\"+rcms_admin_document,
				filePath:\"" . SITE_PATH . "\",
				webAppKey:\"" . $appkey . "\",
				wordCount:false,
		...
		<script type=\"text/javascript\">
			var editorOption = {
			    UEDITOR_HOME_URL: ueditor_url,
				toolbars:[[
				" . $source . "'bold','italic','underline','removeformat','|','fontsize','forecolor','backcolor','|','date', 'time','|','insertimage','map'" . $page . "
				]],
				lang: \"" . SYS_LANGUAGE . "\",
				initialContent:'',
				autoClearinitialContent:false,
...
function content_select($name, $content = '', $field = '') {
    ...
    $select	= explode(chr(10), $setting['content']);
    $str	= "<select id='fc_" . $name . "' name='data[" . $name . "]' " . $required . ">";
    foreach ($select as $t) {
        $n	= $v = $selected = '';
		//$d=explode('|', $t);
		//$n=$d[0];
		//$v=(count($d)>1)?trim($d[0]):trim($d[1]);
        list($n, $v) = explode('|', $t);
...
//取得select的label
function text_select($name, $content = '', $field = '') {
	$setting	= isset($field['setting']) ? string2array($field['setting']) : $field;	//配置
    $content	= is_null($content[0]) ? get_content_value($setting['default']) : $content[0];
	$required	= isset($field['not_null']) && $field['not_null'] ? ' required' : '';	//必填字段
    $select	= explode(chr(10), $setting['content']);
...
function content_radio($name, $content = '', $field = '') {
    ...
    $select	= explode(chr(10), $setting['content']);
    $str	= '';
	$str.= '<!--script type="text/javascript" src="' . JLIB_PATH . 'jqueryui/jquery-ui.min.js"></script-->';
	$str.='<div id="radio_' . $name . '">';
	$i=1;
    foreach ($select as $t) {
        $n	= $v = $selected = '';
        list($n, $v) = explode('|', $t);
		...
		$iid='radio_' . $name .'_'.$i;
		$str.='<input type="radio" id="'.$iid.'" name="data[' . $name . ']" value="' . $v . '" ' . $selected . ' ' . $required . '/><label for="'.$iid.'">'.$n.'</label>';
		$i++;
    }
	$str.= '</div>';
	$str.= '<script type="text/javascript">$(function() {$( "#radio_' . $name . '" ).buttonset();});</script>';
    return $str;
}
//取得radio的label值
function text_radio($name, $content = '', $field = '') {
	$setting	= isset($field['setting']) ? string2array($field['setting']) : $field;	//配置
    $content	= is_null($content) ? get_content_value($setting['default']) : $content;
	$required	= isset($field['not_null']) && $field['not_null'] ? ' required' : '';	//必填字段
    $select	= explode(chr(10), $setting['content']);
    $str	= '';
    foreach ($select as $t) {
...
function content_checkbox($name, $content = '', $field = '') {
    ...
    foreach ($select as $t) {
        ...
        //$str.='&nbsp;<input type="checkbox" name="data[' . $name . '][]" value="' . $v . '" ' . $selected . ' /> $n &nbsp;&nbsp;';
		$iid='check_' . $name .'_'.$i;
		$str.='<input type="checkbox" id="'.$iid.'" name="data[' . $name . '][]" value="' . $v . '" ' . $selected . ' ><label for="'.$iid.'">'.$n.'</label>';
		$i++;
    }
	$str.= '</div>';
	$str.= '<script type="text/javascript">$(function() {$( "#checkbox_' . $name . '" ).buttonset();});</script>';
    return $str;
}
//取得checkbox的label
function text_checkbox($name, $content = '', $field = '') {
	$setting	= isset($field['setting']) ? string2array($field['setting']) : $field;	//配置
    $default	= get_content_value($setting['default']);
    ...
    foreach ($select as $t) {
        ...
        //list($n, $v) = explode('|', $t);
        //$v	= is_null($v) ? trim($n) : trim($v);
        if(is_array($content) && in_array($v, $content)){
			if($str)$str.=",";
			$str.=$n;
		}
    }
...
//显示image
function text_image($name, $content = '', $field = '') {
	$setting	= isset($field['setting']) ? string2array($field['setting']) : $field;	//配置
    $content	= $content[0];
    $size	= (int)$setting['size'];
	$height	= isset($setting['height']) ? $setting['height'] : '';
	$width	= isset($setting['width']) ? $setting['width'] : '';
    $str	= '<input type="button" style="width:66px;cursor:pointer;" class="button" onClick="preview(\'fc_' . $name . '\')" value="' . lang('a-image') . '" />';
    return $str;
}
...
function content_files($name, $content = '', $field = '') {
	...
	$str	= '';
	//if (!defined('RCMS_FILES_LD')) {
	    $str.= '<!--script type="text/javascript" src="' . JLIB_PATH . 'jqueryui/jquery-ui.min.js"></script-->';
		define('RCMS_FILES_LD', 1);//防止重复加载JS
	//}
...
function content_date($name, $content = '', $field = '') {
	...
	//	define('RCMS_DATE_LD', 1);//防止重复加载JS
	//}
	return $str . '
...
//返回日期的text如2012-12-30
function text_date($name, $content = '', $field = '') {
	if($content=="")return "";
	$setting	= isset($field['setting']) ? string2array($field['setting']) : $field;	//配置
    $c	= $content;
	$type  = isset($setting['type']) ? $setting['type'] : '%Y-%m-%d H:M:S';
	//Log::write("fields.php row774 $name --- ".$content);
	return date(str_replace(array('%','M','S'), array('','i','s'), $type), $c);
}

function content_assets($name, $content = '', $field = ''){
	$setting	= isset($field['setting']) ? string2array($field['setting']) : $field;	//配置
	$pattern	= isset($field['pattern']) && $field['pattern'] ? ' pattern="' . $field['pattern'] . '"' : '';	//正则判断
	$required	= isset($field['not_null']) && $field['not_null'] ? ' required' : '';	//必填字段
    $content	= is_null($content[0]) ? get_content_value($setting['default']) : $content[0];
    //return linkageform_area($setting['id'], $content, $name, $setting['level'], $required);
	return assetsform($setting['id'], $content, $name, $setting['level'], $required);
}
//显示联动菜单
function content_linkage($name, $content = '', $field = '',$isAssets=false) {
	$setting	= isset($field['setting']) ? string2array($field['setting']) : $field;	//配置
	$pattern	= isset($field['pattern']) && $field['pattern'] ? ' pattern="' . $field['pattern'] . '"' : '';	//正则判断
	$required	= isset($field['not_null']) && $field['not_null'] ? ' required' : '';	//必填字段
    $content	= is_null($content[0]) ? get_content_value($setting['default']) : $content[0];
    //return linkageform_area($setting['id'], $content, $name, $setting['level'], $required);
	return linkageform($setting['id'], $content, $name, $setting['level'], $required,$isAssets);
}

function text_assets($name, $content = '', $field = ''){
	return text_linkage($name, $content, $field,true);
}
//返回联动菜单值
function text_linkage($name, $content = '', $field = '',$isAssets=false) {	
	//return is_array(json_decode($content))?"array":$content;
	$setting	= isset($field['setting']) ? string2array($field['setting']) : $field;	//配置
	if(is_array(json_decode($content))){
		...
		foreach($ids as $id){
			if (isset($data['data'][$id])) {
				array_push($ret,$data['data'][$id]['name']);
				
				//$idata = $data['data'][$id];
				//array_push($ret,explode("|",$idata['name'])[0]);
			}
		}
		return implode(" , ", $ret);
	}else{
		//$setting	= isset($field['setting']) ? string2array($field['setting']) : $field;	//配置
		$content	= is_null($content) ? get_content_value($setting['default']) : $content;
		return linkagepos($setting['id'], $content, null, '',$isAssets);
	}
}
