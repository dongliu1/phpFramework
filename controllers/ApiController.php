...
class ApiController extends Common {
	...
	public function downAction() {
		...
		$file = isset($data['rcms']) && $data['rcms'] ? $data['rcms'] : '';
		if (empty($file)) {
	...
	public function thumbAction() {
		...
		$file	= isset($data['rcms']) && $data['rcms'] && is_file($data['rcms']) ? $data['rcms'] : EXTENSION_PATH . '/null.jpg';
	...
	public function dataAction() {
	    ...
	    ob_start();
		if (count($data) == 1 && isset($data['rcms_html_to_data'])) {
			$this->view->assign('data', $data['rcms_html_to_data']);
		} else {
	 ...
	 * 联动菜单数据
	 */
	public function linkagedataAction() {
	    $keyid    = (int)$this->get('id');
		$isAssets = ($this->get('isAssets')==1);
	    $parentid = (int)$this->get('parent_id');
	    $linkage  = get_linkage_data($isAssets);
	    $infos    = $linkage[$keyid]['data'];
		echo json_encode($infos,256);	
	}
	
	/**
	 * 联动菜单数据
	 */
	public function linkagelistAction() {
	    $keyid    = (int)$this->get('id');
		$isAssets = ($this->get('isAssets')==1);
	    $parentid = (int)$this->get('parent_id');
	    $linkage  = linkagelist($keyid, $parentid,$isAssets);// get_linkage_data($isAssets);
	    //$infos    = $linkage[$keyid]['data'];
		echo json_encode($linkage,256);	
	}
	...
	public function linkageAction() {
	    $keyid    = (int)$this->get('id');
		$isAssets = ($this->get('isAssets')==true);
		...
		foreach ($infos as $k=>$v) {
			if ($v['parentid'] == $parentid) {
				$json[] = array('id' => $v['id'], 'name' => $v['name']);
			}
		}
	 ...
	 * 联动菜单数据
	 */
	public function linkage_autocompleteAction() {
		...
		foreach ($infos as $k=>$v) {
			...
			if (strpos(strtolower($v['name']), $q) !== false) {
				$json[] = array('id' => $v['id'], 'label' => $v['name'],'value' => $v['id']);
