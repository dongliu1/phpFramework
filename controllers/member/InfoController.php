...
class InfoController extends Member {
	...
	public function editAction() {
		...
		$datas = $this->getFieldsArr($fields, $this->memberinfo);
	...
	public function uploadavatarAction() {
		    ...
		    while(false !== ($file = readdir($handle))) {
				if($file !== '.' && $file !== '..') {
					if(!in_array($file, $avatararr)) {
						copy($temp.$file, $dir.$file);
						//@unlink($temp . $file);
						if (SYS_LOG === true) Log::write("rox avatarAction !in_array --------- ", $level);
					} else {
						$info = @getimagesize($dir . $file);
						if(!$info || $info[2] !=2) {
							if (SYS_LOG === true) Log::write("rox avatarAction info[2] --------- ".$info[2], $level);
							@unlink($temp . $file);
						} else {
							if (SYS_LOG === true) Log::write("rox avatarAction copy --------- ".$dir.$file, $level);
                            copy($temp.$file, $dir.$file);
                            @unlink($temp . $file);
                        }
					}
		...
		//if (SYS_LOG === true) Log::write("rox avatarAction --------- ".$filename, $level);
		@unlink($filename);
        @rmdir($temp);
		//更新用户头像字段
