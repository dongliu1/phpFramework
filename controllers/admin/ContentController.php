...
class ContentController extends Admin {
	...
	public function indexAction() {
	    ...
	    } elseif ($this->post('submit_status_3') && $this->post('form') == 'status_3') {	//标记回收站  status=4是游客信息
			$success = 0;
	        foreach ($_POST as $var => $value) {
	            if (strpos($var, 'del_') !== false) {
	                $ids = str_replace('del_', '', $var);
		...
		if (empty($catid)) $this->adminMsg(lang('a-con-0'));
		
	    $stype    = isset($stype) ? $stype : (int)$this->get('stype');
		$modelid  = (int)$this->cats[$catid]['modelid'];//$this->get('modelid');
		...
		//组装查询条件
		$qcats   = $this->cats[$catid]['arrchildid'];
		//$where  = $catid ? 'catid=' . $catid : 'modelid=' . $modelid;
		$where  = $qcats ? 'catid in (' . $qcats .') ' : ' catid='. $catid;
		//$where  = $qcats ? 'catid in (' . $qcats .') ' : 'modelid=' . $modelid;
		if ($kw && $stype == 0) {
		...
		if ($recycle) {	//回收站
			$count[0]	= $total;
			$total		= (int)$this->content->_count(null, 'catid=' . $catid . ' AND status=1');
		} else {
			$count[0]	= (int)$this->content->_count(null, 'catid=' . $catid . ' AND status=0');
		}
		$count[1]		= (int)$this->verify->count('content_' . $this->siteid . '_verify', null, 'catid=' . $catid . ' AND status=3');	//待审
		$count[2]		= (int)$this->verify->count('content_' . $this->siteid . '_verify', null, 'catid=' . $catid . ' AND status=2');	//拒绝
	...
	public function verifyAction() {
		...
		$modelid  = $this->cats[$catid]['modelid'];//(int)$this->get('modelid');
	    $pagelist = $this->instance('pagelist'); //加载分页类
		$pagelist->loadconfig();
		if (!isset($model[$modelid])) $this->adminMsg(lang('a-con-2', array('1' => $modelid)));
		//模型投稿权限验证
	...
	public function addAction() {
		...
		if (!$catid) $this->adminMsg(lang('a-con-0'));
		...
		$this->view->display(''.(is_file(VIEW_DIR . ADMIN_NAMESPACE . DIRECTORY_SEPARATOR . $model[$modelid]['posttpl']) ? substr($model[$modelid]['posttpl'], 0, -5) : 'content_add'));
	}
	...
	public function ajaxloadinfoAction() {
	    ...
	    $this->view->display('content_data_load');
	}
