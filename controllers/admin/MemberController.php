...
class MemberController extends Admin {
	...
	public function groupAction() {
		...
		switch ($type) {
		    case 'add':
			    ...
			    $this->view->display('member_group_add');
				break;
			case 'edit':
			    $id = (int)$this->get('id');
			    if ($this->isPostForm()) {
				    $data = $this->post('data');
					Log::write("MemberController row-118".json_encode($data['setting'],256));
					if (empty($data['name'])) $this->adminMsg(lang('a-mem-1'));
					if($data['setting'])$data['setting']=array2string($data['setting']);
					if($data['authority'])$data['authority']=array2string($data['authority']);
		    ...
		    default:
				...
				$this->view->display('member_group_list');
		}
	}
    ...
    public function configAction() {
        ...
        $this->view->display('member_config');
	}
	...
	public function pmsAction() {
		...
		switch ($type) {
		    case 'show';
			    ...
			    $this->view->display('member_pms_show');
			    break;
		    case 'send':
			    if ($this->isPostForm()) {
					$type = $this->post('type');
			    ...
			    $this->view->display('member_pms_send');
			    break;
			default:
			    if ($this->isPostForm()) {
					$ids  = $this->post('ids');
			    ...
			    $this->view->display('member_pms_list');
			    break;
		}
	}
