...
class AssetsController extends Admin {
    ...
    public function indexAction() {
	    if ($this->isPostForm()) {
			if (!auth::check($this->roleid, 'assets-del', 'admin')) $this->adminMsg(lang('a-com-0', array('1'=>'assets', '2'=>'del')));
		    $ids = $this->post('ids');
			...
			$this->adminMsg($this->getCacheCode('assets') . lang('success'), url('admin/assets/'), 3, 1, 1);
		}
		$this->view->assign('data', $this->link->where('keyid=0')->select());
		$this->view->display('assets');
    }
	...
	public function listAction() {
		...
		if (empty($keyid)) $this->adminMsg(lang('a-ast-1'));
		if ($this->isPostForm()) {
			...
			$this->adminMsg($this->getCacheCode('assets') . lang('success'), url('admin/assets/list', array('keyid'=>$keyid)));
		}
		...
		$pagelist   = $pagelist->total($total)->url(url('admin/assets/list', array('keyid'=>$keyid, 'parentid'=>$parentid, 'page'=>'{page}')))->num($pagesize)->page($page)->output();
		$sdata      = array();
		foreach($result as $t){
			$sdata[$t['id']] = $t;
			$sdata[$t['id']]['option']  = ($parent && $this->isLastNode($t['keyid'], $t['id'])) ? '<a href="' . url('admin/assets/list', array('keyid'=>$keyid, 'parentid'=>$t['id'])) . '">' . lang('a-ast-0') . '</a> | ' : '';
			!admin_auth($this->userinfo['roleid'], 'assets-addson') or $sdata[$t['id']]['option'] .= '<a href="' . url('admin/assets/addson', array('id'=>$t['id'])) . '">' . lang('a-add') . '</a> | ';
			!admin_auth($this->userinfo['roleid'], 'assets-editson') or $sdata[$t['id']]['option'] .= '<a href="' . url('admin/assets/editson', array('id'=>$t['id'])) . '">' . lang('a-edit') . '</a> | ';
			!admin_auth($this->userinfo['roleid'], 'assets-del') or $sdata[$t['id']]['option'] .= '<a href="javascript:del(' . $t['id'] . ');">' . lang('a-del') . '</a>  ';
		}
		$pdata = $this->link->find($parentid, 'parentid');
		if ($pdata) $pstr = "<a href='" . url('admin/assets/list',array('keyid'=>$keyid,'parentid'=>$pdata['parentid'])) . "'>" . lang('a-ast-2') . "</a> | ";
		$str  = "<tr>
	...
	public function addAction() {
	    if ($this->isPostForm()) {
			$name = $this->post('name');
			if (empty($name)) $this->adminMsg(lang('a-ast-3'));
			$data = array(
				'name' => $name,
				'site' => $this->post('site'),
			);
	...
	public function editAction() {
		...
		if (empty($data)) $this->adminMsg(lang('a-ast-4'));
	    if ($this->isPostForm()) {
			$name = $this->post('name');
			if (empty($name)) $this->adminMsg(lang('a-ast-3'));
			$data = array(
	...
	public function addsonAction() {
	    ...
	    if ($this->isPostForm()) {
			...
			if (empty($names)) $this->adminMsg(lang('a-ast-3'));
			...
			foreach ($names as $name) {
			    $n = trim($name);
			    if (!empty($n)) {
					$sdata = $this->post('data');
					$sdata['keyid']=$keyid;
					$sdata['name']=$n;
					$sdata['site']=$this->siteid;
					$sdata['parentid']=$pid;
				    /*$sdata = array(
					    'keyid'    => $keyid,
						'site'     => $this->siteid,
			...
			$this->adminMsg($this->getCacheCode('assets') . lang('success'), url('admin/assets/list', array('keyid'=>$keyid, 'parentid'=>$pid)), 3, 1, 1);
		}
		//菜单列表
		$tree    = $this->instance('linkage_tree');
	    ...
	    $this->view->display('assets_addson');
	}
	...
	public function editsonAction() {
	    ...
	    if ($this->isPostForm()) {
			...
			if (empty($name)) $this->adminMsg(lang('a-ast-3'));
			$sdata = $this->post('data');
			$sdata['parentid']=$this->post('parentid');
			$sdata['name']=trim($name);
			$this->link->update($sdata, 'id=' . $id);
			$this->link->update(array('child'=>1), 'id=' . $this->post('parentid'));
			$this->adminMsg($this->getCacheCode('assets') . lang('success'), url('admin/assets/list', array('keyid'=>$data['keyid'], 'parentid'=>$this->post('parentid'))), 3, 1, 1);
		}
		//菜单列表
		$tree    = $this->instance('linkage_tree');
		$assets = $this->cache->get('assets');
	...
	public function delAction() {
		$id    = (int)$this->get('id');
		$keyid = $this->get('keyid');
		if (empty($id) || empty($keyid)) $this->adminMsg(lang('a-ast-7'));
		...
		$this->childnode = array();
		$this->link->delete('id=' . $id);
		$this->adminMsg($this->getCacheCode('assets') . lang('success'), url('admin/assets/list', array('keyid'=>$keyid)), 3, 1, 1);
	}
    ...
    public function cacheAction($show=0, $site_id=0) {
		if ($show == 0 && !$this->get('qok')) $this->adminMsg(lang('a-ast-8'), url('admin/assets/cache', array('qok'=>1)), 0, 1, 2);
