...
class Admin extends Common {
    ...
    protected function sysMenu() {
		...
		if ($data) {
			foreach ($data as $t) {
				...
				$menu['list'][5]['a-men-61']['5' . $id] = array('name' => lang($t['name']), 'url' => $url);
				if($this->userinfo['userid']!=1)unset($menu['list'][5]['a-men-61']['501']);
			}
		}
		//$category = $this->cache->getData();
		$category = $this->cache->get('category_' . $this->siteid);
		if ($category) {
		    foreach ($category as $t) {
				if ($this->adminPost($t['setting']['auth'])) continue;
				//if (!$t['ismenu']) continue;
				$imodelid   = $t['modelid'];
				if($imodelid==0){
					$url  = url('admin/category/singlepage_edit', array('catid' => $t['catid']));
					$menu['list'][2]['a-men-29']['cat_'.$t['catid']] = array('name' => lang($t['catname']), 'parentid'=>$t['parentid'],'url' => $url, 'clz' => 1);
				}else{
					//if($t['parentid']==0){
						$url  = url('admin/content/', array('catid'=>$t['catid']));
						//$menu['list'][2]['a-men-29']['281'.$imodelid] = array('name' => $t['catname'], 'url' => $url, 'clz' => 1);
						//$url  = url('admin/category', array('catid' => $t['catid']));
						$menu['list'][2]['a-men-29']['cat_'.$t['catid']] = array('name' => lang($t['catname']),'parentid'=>$t['parentid'], 'url' => $url, 'clz' => 1);
					//}
				}
			}
		...
		//if (SYS_LOG === true) Log::write('Common.php row-100:' . json_encode($category,256));
		return $menu;
    }
    ...
    protected function optionMenu($roleid = 0) {
		...
		//if (SYS_LOG === true) Log::write('Common.php row-217:' . json_encode($menu,JSON_UNESCAPED_UNICODE));
        return $menu;
    }
    ...
    protected function adminLog() {
		...
		file_put_contents($file, date('[Y-m-d H:i:s]')." ::::   ".$content, LOCK_EX); ////rox修改
    }
