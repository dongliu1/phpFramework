...
class ModelController extends Admin {
	...
	public function addAction() {
	    if ($this->post('submit')) {
	        ...
	        $listtpl	= $this->post('listtpl') ? $this->post('listtpl') : 'list_' . $tablename . '.html';
	        $showtpl	= $this->post('showtpl') ? $this->post('showtpl') : 'show_' . $tablename . '.html';
			$listorder	= $this->post('listorder') ? $this->post('listorder') : '100';
	        ...
	        $data	= array(
				...
				'modelcode'		=> $this->post('modelcode'),
				'listorder'		=> $this->post('listorder'),
	        ...
	        if ($modelid = $this->_model->set(0, $data)) { 
			    if ($this->typeid == 1) {	//如果模型类型是数据模型且有关联关系,则更新关联表单
					$join = $this->post('join');
	...
	public function fieldsAction() {
	    ...
	    $this->view->assign(array(
			...
			'isnewsmodel'=>$setting['isnewsmodel'],
			'content' => $setting['default']
		));
	    $this->view->display('model_fields');
	}
	...
	public function addfieldAction() {
	    ...
	    $this->view->display('model_addfield');
	}
	...
	public function ajaxeditAction() {
	    ...
	    $this->view->display('model_ajaxedit');
	}
	...
	public function importAction() {
	    ...
	    $this->view->display('model_import');
	}
	...
	public function cacheAction($show = 0, $site_id = 0) {
		...
		foreach ($this->modeltype as $typeid => $c) {
			...
			//if (SYS_LOG === true) Log::write("ModelController.php row-520".json_encode($model,JSON_UNESCAPED_UNICODE));
	        ...
	        //保存到缓存文件中
			if ($typeid == 2) {
				$this->cache->set('model_member', $data); //会员类别直接保存
			} elseif ($typeid == 4) {
