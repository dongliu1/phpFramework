...
class IndexController extends Admin {
	...
	public function configAction() {
        ...
        if ($this->post('submit')) {
            ...
            $content    = "<?php" . PHP_EOL . "if (!defined('IN_RCMS')) exit();" . PHP_EOL . PHP_EOL . "/**" . PHP_EOL . " * 应用程序配置信息" . PHP_EOL . " */" . PHP_EOL . "return array(" . PHP_EOL .
            ...
            foreach ($config_core as $var=>$msg) {
                ...
                if (is_bool($config[$var])) {
				    ...
				    $value = "'rcms_" . substr(md5(time()), 0, 5) . "_'";
	...
	public function cacheAction() {
		...
		//设备树菜单缓存
		foreach ($sites as $sid => $t) {
			$caches[] = array('31', 'assets',  'cache', array('site' => $sid, 'text' => '(' . $sid . '/' . $count . ')'));
		}
	    if ($this->get('show')) {
	...
	public function updatemapAction() {
	    $fp = @fopen(APP_ROOT . 'rcms_test.txt', 'wb');
		if (!file_exists(APP_ROOT . 'rcms_test.txt') || $fp === false) $this->adminMsg(lang('app-9', array('1' => APP_ROOT)));
		@fclose($fp);
		if (file_exists(APP_ROOT . 'rcms_test.txt')) unlink(APP_ROOT . 'rcms_test.txt');
	    $count = sitemap_xml();
