...
class Common extends Controller {
    ...
    public function __construct() {
		...
		define('ADMIN_THEME',	SITE_PATH . basename(VIEW_DIR) . '/'.ADMIN_PATH.'/');
		define('EXT_PATH',		SITE_PATH . EXTENSION_PATH . '/');
		define('JLIB_PATH',		SITE_PATH . JLIBRARY_PATH . '/');
		...
		$this->view->assign(array(
			...
			'sites'		=> App::get_site(),
			'siteid'	=> $this->siteid,
			'is_admin'	=> $this->session->get('user_id'),
		));
		//加载系统函数库和自定义函数库
    ...
    protected function getFields($fields, $data = array()) {
	    ...
	    foreach ($fields['data'] as $t) {
			...
			if (!@in_array($t['field'], $fields['merge']) && !in_array($t['formtype'], array('merge', 'fields')) && empty($t['merge'])) {
			    //单独显示的字段。
			    $data_fields .= '<tr id="rcms_' . $t['field'] . '" fgroup="'.$t['fgroup'].'">';
				...
				foreach ($contentdata as $i => $cdata) {
					...
					foreach ($fields['data'] as $field => $value) {
						if (in_array($value['field'], $t['data'])) {
							...
							$o_replace_array[]	= str_replace('data[' . $field . ']', 'data[' . $t['field'] . '][{rcms_block_id}][' . $field . ']', $o_str);
						}
					}
					if (empty($merge_string)) {
					    $merge_string = '<li id="li_' . $t['field'] . '_{rcms_block_id}_fields">' . str_replace($regex_array, $o_replace_array, $string) . '<div class="option"><a href="javascript:;" onClick="$(\'#li_' . $t['field'] . '_{rcms_block_id}_fields\').remove()">' . lang('a-mod-129') . '</a> <a href="javascript:;" style="cursor:move;" title="' . lang('a-mod-131') . '">' . lang('a-mod-130') . '</a></div></li>';
				...
				function add_block_' . $t['field'] . '() {
					...
					c = c.replace(/{rcms_block_id}/ig, id);
	...
	* auth   字段权限（是否必填）
	*/
    protected function getFieldsText($fields, $data = array()) {
		App::auto_load('fields');
	    $data_fields = '';
		$data_fgroup="";
		$texts=array();
		...
		foreach($data as $row){
			...
			foreach ($row as $k=>$v) {
				$texts[$rowid][$k]=$v;
				if(isset($fields['data'][$k])){
					$t=$fields['data'][$k];
					if ($this->namespace != 'admin' && !$t['isshow']) continue;
					if (!@in_array($k, $fields['merge']) && !in_array($t['formtype'], array('merge', 'fields')) && empty($t['merge'])) {
						$func         = 'text_' . $t['formtype'];						
						$field_config = var_export($t, true);
						if (function_exists($func)){
							eval("\$texts['" . $rowid . "']['" . $t['field'] . "'] = " . $func . "('" . $k . "', '" . $v . "', " . $field_config . ");");
						}
					} elseif ($t['formtype'] == 'merge') {
						$setting      = string2array($t['setting']);
						$string       = $setting['content'];
						$regex_array  = $replace_array = array();
						foreach ($t['data'] as $field) {
							$ziduan  = $fields['data'][$field];
							$str      = '';
							$func     = 'content_' . $ziduan['formtype'];
							//防止出错，把字段内容转换成数组格式
							$content	= array($data[$field]);
							$content	= var_export($content, true);
							$field_config = var_export($ziduan, true);
							if (function_exists($func)) eval("\$str = " . $func . "(" . $field . ", " . $content . ", " . $field_config . ");");
							$regex_array[]	= '{' . $field . '}';
							$replace_array[] = $str;
						}
						$texts[$t['id']][$t['field']]= str_replace($regex_array, $replace_array, $string);
					} elseif ($t['formtype'] == 'fields') {
						$merge_string = null;
						$contentdata  = empty($data[$t['field']]) ? array(0 => array()) : string2array($data[$t['field']]);
						$setting      = string2array($t['setting']);
						...
						foreach ($contentdata as $i => $cdata) {
							...
							$texts[$t['field']].= str_replace($regex_array, $replace_array, $string);
						}
					}								
				}			
			}
		...
		//Log::write("Common.php row416".json_encode($texts,JSON_UNESCAPED_UNICODE));
	    return $texts;
    }	
	/**
	* 加载自定义字段
	...
	* auth   字段权限（是否必填）
	*/
    protected function getFieldsList($fields, $data = array()) {
	    App::auto_load('fields');
		$data_fields=array();	
		if (empty($fields['data'])) return array();
		$data_fields = array();
	    foreach ($fields['data'] as $t) {
		    if ($this->namespace != 'admin' && !$t['isshow']) continue;
			$data_fields[$t['fieldid']]=array();
			$data_fields[$t['fieldid']]['name']= $t['name'];
			$data_fields[$t['fieldid']]['not_null']=$t['not_null'];
			$data_fields[$t['fieldid']]['tips']=$t['tips'];
			$data_fields[$t['fieldid']]['fgroup']=$t['fgroup'];
			$data_fields[$t['fieldid']]['field']=$t['field'];
			//$data_fields[$t['fieldid']]['value']=$data[$t['field']];
			if (!@in_array($t['field'], $fields['merge']) && !in_array($t['formtype'], array('merge', 'fields')) && empty($t['merge'])) {
			    //单独显示的字段。
			    //$data_fields .= '<tr id="rcms_' . $t['field'] . '" fgroup="'.$t['fgroup'].'">';
				$func         = 'content_' . $t['formtype'];
				...
				$field_config = var_export($t, true);
				$data_content ="";
				if (function_exists($func)) eval("\$data_content = " . $func . "(" . $t['field'] . ", " . $content . ", " . $field_config . ");");
				$data_fields[$t['fieldid']]['content']=$data_content;
			} elseif ($t['formtype'] == 'merge') {
				$setting      = string2array($t['setting']);
				$string       = $setting['content'];
				$regex_array  = $replace_array = array();
				...
				$data_fields[$t['fieldid']]['content']= str_replace($regex_array, $replace_array, $string);
			} elseif ($t['formtype'] == 'fields') {

				$string .= '<div class="fields-list" id="list_' . $t['field'] . '_fields"><ul id="' . $t['field'] . '-sort-items">';
				$merge_string = null;
				$contentdata  = empty($data[$t['field']]) ? array(0 => array()) : string2array($data[$t['field']]);
				$setting      = string2array($t['setting']);
				$string       = htmlspecialchars_decode($setting['content']);
				foreach ($contentdata as $i => $cdata) {
				    $string .= '<li id="li_' . $t['field'] . '_' . $i . '_fields">';
				    $regex_array  = $replace_array = $o_replace_array = array();
					foreach ($fields['data'] as $field => $value) {
						if (in_array($value['field'], $t['data'])) {
							$str  = $o_str  = '';
					...
					$string .= str_replace($regex_array, $replace_array, $string);
					$string .= '<div class="option"><a href="javascript:;" onClick="$(\'#li_' . $t['field'] . '_' . $i . '_fields\').remove()">' . lang('a-mod-129') . '</a> <a href="javascript:;" style="cursor:move;" title="' . lang('a-mod-131') . '">' . lang('a-mod-130') . '</a></div></li>';
				}
				$string .= '</ul>
				<div class="bk10"></div>
				<div class="picBut cu"><a href="javascript:;" onClick="add_block_' . $t['field'] . '()">' . lang('a-add') . '</a></div> 
				...
				$data_fields[$t['fieldid']]['content']=  $string;
			}
	    }
	...
	* auth   字段权限（是否必填）
	*/
    protected function getFieldsArr($fields, $data = array()) {
	    App::auto_load('fields');
		$datas=array();		
		$data_fgroup = '';
		$data_fields = '';
	    ...
	    foreach ($fields['data'] as $t) {
			if(!$t['fgroup']){
				$data_fgroup="系统";
			}elseif($t['fgroup']!=$data_fgroup){
				//if($data_fgroup)$datas[$data_fgroup]=$data_fields;
				if($data_fgroup)$datas[$data_fgroup]=$data_fields;
				$data_fields = '';
				$data_fgroup=$t['fgroup'];
			}
	...
	protected function memberPost($data) {
		//if (isset($data['siteuser']) && $data['siteuser'] && $data['site'] && in_array($this->siteid, $data['site'])) return false;
		//if (isset($data['modelpost']) && in_array($this->memberinfo['modelid'], $data['modelpost'])) return true;
		//Log::write(json_encode($data,JSON_UNESCAPED_UNICODE)." -- ".$this->memberinfo['modelid']." -- ".(isset($data['modelpost'])));
		if(!is_array($data)) return false;
		if($data['allowpost']){
			return true;
		}elseif ($data['grouppost'][$this->memberinfo['groupid']]['allowpost']){	
			return true;
		}
		/*
		if (isset($data['memberpost']) && $data['memberpost']) {
			//Log::write(json_encode($data,JSON_UNESCAPED_UNICODE)." -- ".$this->memberinfo['modelid']." -- ".(isset($data['modelpost']) &&in_array($this->memberinfo['modelid'], $data['modelpost'])));
		    if (isset($data['modelpost']) && in_array($this->memberinfo['modelid'], $data['modelpost'])) return true;
			//if (isset($data['modelpost']) && $data['modelpost']["count_".$this->memberinfo['modelid']]>0) return true;
			//$data['modelpost']['member_'.$this->memberinfo['modelid'].'_daily']
     ...
     * 会员转载权限判断 rox修改
     */
	protected function memberReprint($data) {
		if (isset($data['siteuser']) && $data['siteuser'] && $data['site'] && in_array($this->siteid, $data['site'])) return true;
		if (isset($data['reprint']) && $data['reprint']) {
		    if (isset($data['reprintrole']) && in_array($this->memberinfo['modelid'], $data['reprintrole'])) return true;
		}
		return false;
	}
