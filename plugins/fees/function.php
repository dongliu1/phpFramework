...
function fees_before($data) {
	...
	if ($member && $config[$data['catid']]['model'] && @in_array($member['modelid'], $config[$data['catid']]['model'])) return false;	//判断会员类别
	...
	$pay	= App::plugin_model('fees', 'Pay_data');	//实例化插件模型
...
function fees_later($data) {
	...
	if (empty($money) || $money == 0.00) return false;	//扣款资金数判断
	$pay	= App::plugin_model('fees', 'Pay_data');	//实例化插件模型
	$spend	= App::plugin_model('fees', 'Pay_spend');	//实例化插件模型
