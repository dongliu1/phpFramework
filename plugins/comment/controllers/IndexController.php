...
class IndexController extends Plugin {
	...
	public function delAction() {
		...
		if (empty($cid)) exit(json_encode(array('status'=>0, 'result'=>'参数不正确！')));
		$data = $this->comment_data->find($cid, 'id,commentid,reply,userid,username,contentid');
		if (empty($data)) exit(json_encode(array('status'=>0, 'result'=>'评论信息不存在！')));
		if (($this->session->is_set('user_id') && $this->session->get('user_id')) || (isset($this->memberinfo['id']) && ($this->memberinfo['id'] == $data['userid'] && $this->memberinfo['username'] == $data['username']))) {
			if ($data['reply']) {
	...
	public function ipaddressAction() {
	    $ip = $this->get('ip');
		if (empty($ip)) exit('无效IP');
        $re = fn_geturl('http://www.rcms.net/index.php?c=sys&a=ipaddress&ip=' . $ip);
        echo $re ? $re : '查询超时';		
	}
