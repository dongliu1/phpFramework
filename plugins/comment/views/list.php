...
function load_comment(num) {
	$('#rcms_comment_list').html('<div align=center style="padding-top:10px;font-size:12px;">评论加载中....</div>');
	$.get('<?php echo purl('index/clist/');?>&'+Math.random(), {id:<?php echo $commentid?>, num:num},function(data){
	    if (data) {
			$('#rcms_comment_list').html(data);
		} else {
			$('#rcms_comment_list').html('');
		}
	});
}
function KeyDown(event, id) {
	if (event.keyCode == 13) {
		if (id == '') {
		    if ($('#rcms_comment_ispost').val() == 1) {
			    alert('亲，动作不要太快了哦~');
			} else {
			    comment_post();
			}
		} else {
		    if ($('#rcms_comment_isreply').val() == 1) {
			    alert('亲，动作不要太快了哦~');
			} else {
			    comment_reply(id);
			}
...
function change_more(type) {
	if (type == 1) {
		$('#rcms_commnet_more').attr('style', 'cursor: pointer; background:none repeat scroll 0 0 #e4e6e4;height: 28px; line-height: 28px; display: block; border: 1px solid rgb(220, 220, 220); margin-top: 15px; text-align: center; border-radius: 3px 3px 3px 3px; font-size: 13px; color: rgb(66, 66, 66); text-decoration: none;');
	} else {
		$('#rcms_commnet_more').attr('style', 'cursor: pointer; background: url(&quot;<?php echo $viewpath?>images/morebg.png&quot;) repeat-x scroll 0% 0% transparent; height: 28px; line-height: 28px; display: block; border: 1px solid rgb(220, 220, 220); margin-top: 15px; text-align: center; border-radius: 3px 3px 3px 3px; font-size: 13px; color: rgb(66, 66, 66); text-decoration: none;');
	}
}
function load_more_comment() {
	var num = $('#rcms_commnet_more').attr('limit');
	num = parseInt(num) + 3;
	load_comment(num);
}
	...
	if (content != '') {
		...
		function(data) {
			if (data.status == 1) {
				...
				if (data.verify == 1) {
					$('#rcms_comment_reply_'+id+'_result').html(data.result+$('#rcms_comment_reply_'+id+'_result').html());
					show_comment_reply(id);
				}
			} else {
				result.html('<b><font color=red>'+data.data+'</font></b>');
			}
			$('#rcms_comment_isreply').val('0');
		});
	} else {
...
function comment_cd(id, type) {
	$.getJSON('<?php echo purl('index/cd/');?>&'+Math.random(),{commentid:id,type:type},
		function(data) {
			if (data.status) {
				if (type == 1) {
					$('#rcms_comment_ding_'+id).html(data.result);
				} else {
					$('#rcms_comment_cai_'+id).html(data.result);
				}
			} else {
				alert(data.result);
			}
...
<div id="rcms_comment_frame">
<input type="hidden" id="rcms_comment_ispost" value="">
<input type="hidden" id="rcms_comment_isreply" value="">
    <div style="width: 100%; font-family: 'Microsoft YaHei'; position: relative; float: left;">
        <div style="height: 30px; font-size: 12px; font-weight: bold;" id="rcms_comment_top">
