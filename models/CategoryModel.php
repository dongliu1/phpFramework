...
class CategoryModel extends Model {
	...
	public function getBaseData($site = 0) {
		return $this->where('site=' . $this->getSiteId($site))->order('listorder ASC')->from(null, "catid,catname,parentid,setting,arrparentid,arrchildid");
	}	
	...
	public function set($catid, $data) {
	    ...
	    if ($catid) {
			...
			if ($data['synpost']) {
				...
				if (count($childs) > 2) {
					foreach ($childs as $id) {
						...
						$cset['reprint']	= $data['setting']['reprint'];
						$cset['reprintrole']	= $data['setting']['reprintrole'];
						$cset['reprinttimes']	= $data['setting']['reprinttimes'];
			...
			if (!empty($data['parentid']) && empty($data['child'])) {
				...
				$data['setting']['reprint']    = $data['setting']['reprint']    ? $data['setting']['reprint']    : ($pset['reprint']    ? $pset['reprint']    : null);
				$data['setting']['reprintrole']= $data['setting']['reprintrole']? $data['setting']['reprintrole']: ($pset['reprintrole']? $pset['reprintrole']: null);
				$data['setting']['reprinttimes']= $data['setting']['reprinttimes']? $data['setting']['reprinttimes']: ($pset['reprinttimes']? $pset['reprinttimes']: null);
				unset($pdata, $pset);
			}
			unset($data['synpost']);
			$data['setting'] = array2string($data['setting']);
	...
	public function repair($parentid = 0) {
		...
		$data = $this->where('site=' . $this->getSiteId()  )->select();
	    foreach ($data as $t) {
	        //检查该栏目下是否有子栏目
	        $catid       = $t['catid'];
	        $parentid    = $t['parentid'];

			$arrparentid=$this->get_arr_parent($catid);
			$arr_child=array();
			$rs_child=$this->where("arrparentid like '%". $catid . ",". $arrparentid. "'")->order('listorder asc')->select();
			
			foreach ($rs_child as $r) {$arr_child[]=$r['catid'];}
			
			$arrchildid = implode(',',$arr_child);
			$this->update(array('child' => !(empty($arrchildid)), 'arrchildid' => $arrchildid ,'arrparentid' =>$arrparentid), 'catid=' . $catid);
	        ...
	        if ($s_data) { //存在子栏目
	            ...
	            //$this->update(array('child' => 1, 'arrchildid' => $arrchildid, 'arrparentid' => $this->get_arr_parent($catid)), 'catid=' . $catid);
				//$this->update(array('child' => 1, 'arrchildid' => $arrchildid), 'catid=' . $catid);
	            $this->repair($catid); //递归调用
	        } else { //没有子栏目
	            //$this->update(array('child' => 0, 'arrchildid' => '', 'arrparentid' => $arrparentid), 'catid=' . $catid);
				//$this->update(array('child' => 0, 'arrchildid' => '', 'arrparentid' => $this->get_arr_parent($catid)), 'catid=' . $catid);
	        }*/
	    } 
	}
	
	private function get_arr_parent($catid){
		$iparent="";
		$iparentid=$catid;	
		$arr_parent=array();		
		while($iparentid!=0){					
			$iparentid = $this->getFirst("catid =".$iparentid,NULL,"parentid");			
			$arr_parent[]=$iparentid;
		}
		return join(",",$arr_parent);
	}
