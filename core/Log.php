...
class Log extends Fn_base {
	...
	public static function write($message, $level = 'Error', $log_file_name = null) {
		...
		if (is_null($log_file_name)) $log_file_name = APP_ROOT . 'cache' . DIRECTORY_SEPARATOR . 'logs' . DIRECTORY_SEPARATOR . date('Ymd', $_SERVER['REQUEST_TIME']) . '.log';
		if (is_file($log_file_name) && filesize($log_file_name) >= 2097152) {			
			rename($log_file_name, APP_ROOT . 'cache' . DIRECTORY_SEPARATOR . 'logs' . DIRECTORY_SEPARATOR . $_SERVER['REQUEST_TIME'] . '-' . basename($log_file_name));
		}
	...
	public static function show($log_file_name = null) {
		$log_dir=APP_ROOT . 'cache' . DIRECTORY_SEPARATOR . 'logs' . DIRECTORY_SEPARATOR ;
		$log_file_name =  is_null($log_file_name) ?$log_dir. date('Ymd', $_SERVER['REQUEST_TIME']) . '.log' : $log_dir . $log_file_name . '.log';
