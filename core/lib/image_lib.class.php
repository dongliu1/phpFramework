...
class image_lib extends Fn_base {
	...
	public function make_limit_image($url, $dist_name = null) {
		...
		$image_dist = imagecreatetruecolor($this->width, $this->height);
		$color=imagecolorallocate($image_dist,255,255,255);
		imagecolortransparent($image_dist,$color);
		imagefill($image_dist,0,0,$color);
		//生成新图片.  rox修改
		$dst_x=(int)($this->width-$this->width_new)/2;
		$dst_y=(int)($this->height-$this->height_new)/2;
		imagecopyresampled($image_dist, $this->image, $dst_x, $dst_y, 0, 0, $this->width_new, $this->height_new, $this->image_width, $this->image_height);
		//imagecopyresampled($image_dist, $this->image, 0, 0, 0, 0, $this->width, $this->height, $this->image_width, $this->image_height);
		$this->create_image($image_dist, $dist_name, $this->type);
		//$this->create_image($image_dist, $dist_name, "png");
	...
	public function make_text_watermark($image_url, $pos = null, $size = 14) {
		...
		$this->text_content = empty($this->text_content) ? 'RCMS' : $this->text_content;
