...
class cache_file extends Fn_base {
	...
	public function set($key, $value) {
		...
		$value = (!is_array($value)) ? json_encode(trim($value),JSON_UNESCAPED_UNICODE) : json_encode($value,JSON_UNESCAPED_UNICODE);
		//分析缓存目录
	 ...
	 * @return string
	 */
	public function get($key,$isarray=true) {
		
		if (!$key) return false;
		//分析缓存文件
		$cache_file = $this->parse_cache_file($key);
		//return is_file($cache_file) ? unserialize(file_get_contents($cache_file)) : false;
		return is_file($cache_file) ? json_decode(file_get_contents($cache_file),$isarray) : false;
	}
