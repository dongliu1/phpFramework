		...
		foreach ($data as $key => $value) {
			...
			if ($value[$this->parentid] == $parent_id) {
				...
				if ($value['child'] == 1) {
				    ...
				    //会员权限判断v1.6
					if ($value['setting']['modelpost'] && !in_array($_modelid, $value['setting']['modelpost'])) {//rox修改 if ($value['setting']['modelpost'] && in_array($_modelid, $value['setting']['modelpost'])) {
						if ($value['child'] == 0) continue;
					    $str .= ' disabled';
					} elseif ($value['setting']['grouppost'] && !in_array($_groupid, $value['setting']['grouppost'])) {//rox修改} elseif ($value['setting']['grouppost'] && in_array($_groupid, $value['setting']['grouppost'])) {
						if ($value['child'] == 0) continue;
						$str .= ' disabled';
					}
