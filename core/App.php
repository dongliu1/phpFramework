...
define('JLIBRARY_PATH',			'jlibs');                                     	//extension目录文件夹
...
define('SYS_VAR_PREX',			isset($config['SYS_VAR_PREX']) && $config['SYS_VAR_PREX'] ? $config['SYS_VAR_PREX'] : 'rcms_');	//SESSION和COOKIE变量前缀
 ...
 * 语言调用函数
 */
function lang($word, $data = null) {
	//Log::write(" App.php row-437 " . $word);
    $language = App::get_language();
	//Log::write(" App.php row-437".json_encode($language,256));
	//Log::write(" App.php row-440 " . $language[$word]);
	$string   = isset($language[$word]) ? $language[$word] : $word;
	if ($data) {
		foreach ($data as $r => $t) {
