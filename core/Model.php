...
abstract class Model extends Fn_base {
	...
	public function getFirst($where, $value = null, $fields = null) {
		if (!$where) return false;
		//获取数据表名
		$this->get_table_name();
		//分析查询的字段信息		
		...
		$where_str = $this->_parts['where'];
		unset($this->_parts['where']);
		$sql = 'SELECT ' . $fields_str . ' FROM ' . $this->table_name . $where_str;
	...
	public function insert($data) {
		...
		foreach ($data as $key=>$value) {
			if (in_array($key, $this->table_field)) {
				...
				if(is_array($value))$value=array2string($value);//rox修改
		...
		//Log::write("membercontroller.php row750".$sql_str);
