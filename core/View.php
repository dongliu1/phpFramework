...
class View extends Fn_base {
	...
	public function __construct() {
		...
		//Log::write("View.php rwo-21 ::::: ".json_encode(APP::get_namespace_id(),JSON_UNESCAPED_UNICODE));
		//$this->theme = APP::get_namespace_id() == 'admin' ? false : true;
		//$this->theme = APP::get_namespace_id() == '' ? true : false;		//////////////rox
		$this->theme = APP::get_namespace_id() == '' ? SYS_THEME_DIR : (APP::get_namespace_id().DIRECTORY_SEPARATOR);
		//Log::write("View.php rwo-23 ::::: ".json_encode($this->theme,JSON_UNESCAPED_UNICODE));
	...
	protected function parse_file_name($file_name = null) {
		...
		//return $this->theme ? SYS_THEME_DIR . $file_name : $file_name;  //////////////rox
		return $this->theme . $file_name ;
	}
	...
	protected function handle_view_file($view_content) {
		...
		//$regex_array = array('/{lang\([\'_\"](.+?)[\'_\"]\)}/is');
		//$replace_array=array(lang("\\1"));//{lang(
		...
		'#'.$this->left_delimiter.'\s?foreach\s+\$(.+?)\s+\$(\w+?)\s?'.$this->right_delimiter.'#i',
		'#'.$this->left_delimiter.'\s?foreach\s+\$(.+?)\s+\$(\w+?)\s?=>\s?\$(\w+?)\s?'.$this->right_delimiter.'#i',
			...
			if ($mod['fields']) {
			    foreach ($mod['fields']['data'] as $t) {
				    if ($fields['name'] == $t['field']) {
						...
						$data_fields     = '';
						$data_fields	.= '<tr id="rcms_' . $t['field'] . '">';
						...
						if ($t['setting']) {
							...
							if ($c['content']) {
							    $select  = explode(chr(10), $c['content']);
								$vdata   = array();
