(function() {
					...
					if (e !== "" || g.length === 1) {
						...
						if (e !== " ") {
							...
							if (l) {
								...
								(o || v) && (n.length || u.length);) a.rotation = 0, S = a.getBBox(!0), q = S.width, !ca && c.forExport && (q = c.measureSpanWidth(s.firstChild.data, a.styles)), S = q > l, w === void 0 && (w = S), v && w ? (C /= 2, z === "" || !S && C < 0.5 ? n = [] : (S && (w = !0), z = e.substring(0, z.length + (S ? -1 : 1) * ta(C)), n = [z + (l > 3 ? "бн" : "")], s.removeChild(s.firstChild))) : !S || n.length === 1 ? (n = u, u = [], n.length && (r++, s = B.createElementNS(Ca, "tspan"), J(s, {
									dy: y,
									x: j
	...
	if (!ca && !fa) {
		F = {
			...
			pathToVML: function(a) {
				for (var b = a.length, c = []; b--;) if (ra(a[b])) c[b] = r(a[b] * 10) - 5;
				else if (a[b] === "Z") c[b] = "x";
		...
		var Na = {
			...
			color: function(a, b, c, d) {
				...
				if (i) {
					...
					if (c === "fill") if (i === "gradient") c = m.x1 || m[0] || 0, a = m.y1 || m[1] || 0, n = m.x2 || m[2] || 0, m = m.y2 || m[3] || 0, t = 'angle="' + (90 - W.atan((m - a) / (n - c)) * 180 / ma) + '"', q();
					else {
						var j = m.r,
							u = j * 2,
							y = j * 2,
	...
	va.prototype = {
		...
		defaultLabelFormatter: function() {
			...
			else if (f && a >= 1E3) for (; f-- && g === y;) c = Math.pow(1E3, f + 1), a >= c && b * 10 % c === 0 && e[f] !== null && (g = A.numberFormat(b / c, -1) + e[f]);
			...
			o(a.series, function(c) {
				if (c.visible || !b.options.chart.ignoreHiddenSeries) {
					...
					if (a.isXAxis) {
						...
						if (q(d)) if (a.dataMin >= d) a.dataMin = d, a.ignoreMinPadding = !0;
		...
		setTickInterval: function(a) {
			...
			if (e)!a && C(b.min, p(b.dataMin, b.min)) <= 0 && la(10, 1), b.min = ea(Ea(b.min)), b.max = ea(Ea(b.max));
	...
	R.prototype = {
			...
			if (d !== !1 && h && g === h && !e.cropped && !e.hasGroupedData && e.visible) o(a, function(a, b) {
				...
				if (n && h > n) {
					...
					if (ra(k)) {
						...
						e.xIncrement = m
					} else if (Ha(k)) if (q) for (c = 0; c < h; c++) i = a[c], v[c] = i[0], s[c] = i.slice(1, q + 1);
					else for (c = 0; c < h; c++) i = a[c], v[c] = i[0], s[c] = i[1];
					else la(12)
		...
		processData: function(a) {
			...
			if (k && this.sorted && (!i || d > i || this.forceCrop)) if (b[d - 1] < l || b[0] > m) b = [], c = [];
		...
		generatePoints: function() {
			...
			for (m = 0; m < g; m++) i = h + m, j ? l[m] = (new f).init(this, [d[m]].concat(sa(e[m]))) : (b[i] ? k = b[i] : a[i] !== y && (b[i] = k = (new f).init(this, a[i], d[m])), l[m] = k), l[m].index = i;
		...
		drawPoints: function() {
			...
			if (l.enabled !== !1 || this._hasPointMarkers) for (f = b.length; f--;) if (g = b[f], d = V(g.plotX), e = g.plotY, k = g.graphic, n = g.marker || {}, v = !! g.marker, a = q && n.enabled === y || n.enabled, o = g.isInside, a && e !== y && !isNaN(e) && g.y !== null) if (a = g.pointAttr[g.selected ? "select" : ""] || m, h = a.r, i = p(n.symbol, this.symbol), j = i.indexOf("url") === 0, k) k[o ? "show" : "hide"](!0).animate(x({
				x: d - h,
				y: e - h
			}, k.symbolName ? {
				width: 2 * h,
	...
	xa = ka(R, {
		...
		getColumnMetrics: function() {
			...
			b.grouping === !1 ? i = 1 : o(a.chart.series, function(b) {
				var c = b.options,
					e = b.yAxis;
				if (b.type === a.type && b.visible && d.len === e.len && d.pos === e.pos) c.stacking ? (f = b.stackKey, g[f] === y && (g[f] = i++), h = g[f]) : c.grouping !== !1 && (h = i++), b.columnIndex = h
			});
			...
			for (F = 2; F--;) {
				...
				if (K) {
					...
					for (G = 0; G < K; G++) {
						...
						y.connX = B;
						y.connY = A;
						if (this.options.size === null) t = y.width, B - t < f ? E[3] = u(r(t - B + f), E[3]) : B + t > h - f && (E[1] = u(r(B + t - h + f), E[1])), A - b / 2 < 0 ? E[0] = u(r(-A + b / 2), E[0]) : A + b / 2 > i && (E[2] = u(r(A + b / 2 - i), E[2]))
					}
				}
			}
...
(function(k, D) {
		...
		O = {
			...
			getPlotBandPath: function(a, b, c) {
				var d = this.center,
					...
					g = /%$/,
					l, m = this.isCircular;
				this.options.gridLineInterpolation === "polygon" ? d = this.getPlotLinePath(a).concat(this.getPlotLinePath(b, !0)) : (a = Math.max(a, this.min), b = Math.min(b, this.max), m || (i[0] = this.translate(a), i[1] = this.translate(b)), i = R(i, function(a) {
			...
			getPlotLinePath: function(a, b) {
				var c = this,
					...
					f = c.getPosition(a),
					i, g, l;
				c.isCircular ? l = ["M", d[0] + e.plotLeft, d[1] + e.plotTop, "L", f.x, f.y] : c.options.gridLineInterpolation === "circle" ? (a = c.translate(a)) && (l = c.getLinePath(0, a)) : (t(e.xAxis, function(a) {
					a.pane === c.pane && (i = a)
	...
	u(y, "getLabelPosition", function(a, b, c, d, e, f, i, g, l) {
		...
		m.isRadial ? (a = m.getPosition(this.pos, m.center[2] / 2 + q(f.distance, -25)), f.rotation === "auto" ? d.attr({
			...
			align: h
		})), a.x += f.x, a.y += j) : a = a.call(this, b, c, d, e, f, i, g, l);
		return a
	});
	...
	h.arearange = v(h.area, {
		...
		drawDataLabels: function() {
			var a = this.data,
				...
				i = f.align,
				g, l, m = this.chart.inverted;
			if (f.enabled || this._hasPointLabels) {
				for (c = b; c--;) if (g = a[c]) if (l = g.plotHigh > g.plotLow, g.y = g.high, g._plotY = g.plotY, g.plotY = g.plotHigh, d[c] = g.dataLabel, g.dataLabel = g.dataLabelUpper, g.below = l, m) {
	...
	h.boxplot = v(h.column, {
		...
		drawPoints: function() {
			var a = this,
				...
				c = a.options,
				d = a.chart.renderer,
				e, f, i, g, l, m, j, n, h, A, k, J, p, o, u, r, v, s, w, x, z, y, F = a.doQuartiles !== !1,
			...
			t(b, function(b) {
				...
				r = {};
				y = b.color || a.color;
				if (b.plotY !== D) if (e = b.pointAttr[b.selected ? "selected" : ""], v = z.width, s = B(z.x), w = s + v, x = E(v / 2), f = B(F ? b.q1Plot : b.lowPlot), i = B(F ? b.q3Plot : b.lowPlot), g = B(b.highPlot), l = B(b.lowPlot), k.stroke = b.stemColor || c.stemColor || y, k["stroke-width"] = q(b.stemWidth, c.stemWidth, c.lineWidth), k.dashstyle = b.stemDashStyle || c.stemDashStyle, o.stroke = b.whiskerColor || c.whiskerColor || y, o["stroke-width"] = q(b.whiskerWidth, c.whiskerWidth, c.lineWidth), r.stroke = b.medianColor || c.medianColor || y, r["stroke-width"] = q(b.medianWidth, c.medianWidth, c.lineWidth), j = k["stroke-width"] % 2 / 2, n = s + x + j, A = ["M", n, i, "L", n, g, "M", n, f, "L", n, l], F && (j = e["stroke-width"] % 2 / 2, n = B(n) + j, f = B(f) + j, i = B(i) + j, s += j, w += j, J = ["M", s, i, "L", s, f, "L", w, f, "L", w, i, "L", s, i, "z"]), C && (j = o["stroke-width"] % 2 / 2, g += j, l += j, p = ["M", n - x * C, g, "L", n + x * C, g, "M", n - x * C, l, "L", n + x * C, l]), j = r["stroke-width"] % 2 / 2, m = E(b.medianPlot) + j, u = ["M", s, m, "L", w, m], h) b.stem.animate({
					d: A
	...
	h.waterfall = v(h.column, {
		...
		translate: function() {
			var a = this.options,
				b = this.yAxis,
				c, d, e, f, i, g, l, m, j, n = a.threshold,
				k = a.stacking;
			h.column.prototype.translate.apply(this);
			l = m = n;
			d = this.points;
		...
		getGraphPath: function() {
			var a = this.data,
				...
				d = [],
				e, f, i;
			for (i = 1; i < b; i++) f = a[i].shapeArgs, e = a[i - 1].shapeArgs, f = ["M", e.x + e.width, e.y + c, "L", f.x, e.y + c], a[i - 1].y < 0 && (f[2] += e.height, f[5] += e.height), d = d.concat(f);
			return d
		},
		getExtremes: r,
	...
	h.bubble = v(h.scatter, {
		...
		getRadii: function(a, b, c, d) {
			var e, f, i, g = this.zData,
				l = [],
				m = this.options.sizeBy !== "width";
			for (f = 0, e = g.length; f < e; f++) i = b - a, i = i > 0 ? (g[f] - a) / (b - a) : 0.5, m && i >= 0 && (i = Math.sqrt(i)), l.push(w.ceil(c + i * (d - c)) / 2);
			this.radii = l
		},
		animate: function(a) {
	...
	(function() {
		...
		h.spline && u(h.spline.prototype, "getPointSpline", function(a, b, c, d) {
			...
			if (this.chart.polar) {
				...
				if (a && n) h = a.plotX, k = a.plotY, b = n.plotX, p = n.plotY, h = (1.5 * e + h) / 2.5, k = (1.5 * j + k) / 2.5, n = (1.5 * e + b) / 2.5, o = (1.5 * j + p) / 2.5, b = Math.sqrt(Math.pow(h - e, 2) + Math.pow(k - j, 2)), p = Math.sqrt(Math.pow(n - e, 2) + Math.pow(o - j, 2)), h = Math.atan2(k - j, h - e), k = Math.atan2(o - j, n - e), o = Math.PI / 2 + (h + k) / 2, Math.abs(h - o) > Math.PI / 2 && (o -= Math.PI), h = e + Math.cos(o) * b, k = j + Math.sin(o) * b, n = e + Math.cos(Math.PI + o) * p, o = j + Math.sin(Math.PI + o) * p, c.rightContX = n, c.rightContY = o;
