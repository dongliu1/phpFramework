	...
	var mapsize={"width":800,"height":700};
	var myinfo={"id":"china","width":800,"height":650,"type":"json","scale":1000};
	var addressId=["china"];
	//var submaps=[];
	$(function () {
		draw_map();				//绘出地图		
		$(".skin-light .knob").knob({'min':0,'max':100,step:0.1,'fgColor':'#529c78','bgColor':'#ccc'});	//初始化百分比圆框
		$(".knob").knob({'min':0,'max':100,step:0.1});	//初始化百分比圆框
		render_statics_chart();	//绘出统计柱图
		init_return_btn();		//初始化返回按钮
	});
	//打开关闭的悬浮窗口
	function open_windows(cpid){
		var open_id,cur_title;
		switch(cpid){
			case "cpid_1":
				open_id="#station-info";
				title="清水河";
				break;
			case "cpid_2":
				open_id="#gener-info";
		...
		var dialog_title={title:cur_title};
		$(open_id).dialog(dialog_title);
	}
	//初始化返回按钮
	var init_return_btn=function(){
	...
	function render_statics_chart(){
	    ...
	    $('#barChart').highcharts({
			...
			tooltip: {
				...
				pointFormat: '<tr><td style="background:{series.color};">&nbsp;</td><td>{series.name}: </td>' +
	...
	var init_svg_paper=function(containerid){//xmlns="http://www.w3.org/2000/svg"
		...
		var projection=nrender_map("svg_basemap");				//渲染底图
	...
	var nrender_map=function(containerid){
		...
		.scale(scale*1).translate(offset);
